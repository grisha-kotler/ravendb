            ...
            using (status.LocalBackup.UpdateStats(_isFullBackup))
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        var compressionLevel = Configuration.SnapshotSettings?.CompressionLevel ?? CompressionLevel.Optimal;
                        ...
                        using (var stream = GetStreamForBackupDestination(tempBackupFilePath, folderName, fileName))
                        {
                            try
                            {
                                var smugglerResult = Database.FullBackupTo(stream, compressionLevel, excludeIndexes,
                                    info =>
                                    {
        ...
        private InternalBackupResult CreateBackup(DatabaseSmugglerOptionsServerSide options, string backupFilePath, string folderName, string fileName, long? startDocumentEtag, long? startRaftIndex)
        {
            ...
            using (Database.DocumentsStorage.ContextPool.AllocateOperationContext(out DocumentsOperationContext context))
            {
                try
                {
                    ...
                    var smugglerDestination = new StreamDestination(outputStream, context, smugglerSource, Database.Configuration.Backup.CompressionAlgorithm);

...
import monitorCompactCommand = require("commands/counter/monitorCompactCommand");
...
class startCompactCommand extends commandBase {
    ...
    execute(): JQueryPromise<any> {
        ...
        new deleteDocumentCommand("Raven/Database/Counters/Status/" + this.fsToCompact, this.db)
            .execute()
            ...
            .done(_=> {
                var url = "/admin/cs/compact" + this.urlEncodeArgs({ filesystem: this.fsToCompact });
                this.post(url, null, this.db)
                    .fail((response: JQueryXHR) => {
                        this.reportError("Failed to compact counter storage!", response.responseText, response.statusText);
